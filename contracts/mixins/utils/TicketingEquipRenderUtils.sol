...
import "./TicketingRenderUtils.sol";
import "./TicketingMultiAssetRenderUtils.sol";
import "./TicketingNestableRenderUtils.sol";

/**
 * @title TicketingEquipRenderUtils
 *
 * @notice Smart contract of the Ticketing Equip render utils module.
 * @dev Extra utility functions for composing Ticketing extended assets.
 */
contract TicketingEquipRenderUtils is
    TicketingRenderUtils,
    TicketingMultiAssetRenderUtils,
    TicketingNestableRenderUtils
{
    ...
    function getExtendedEquippableActiveAssets(
        ...
        if (len == uint256(0)) {
            revert TicketingTokenHasNoAssets();
        }
        ...
        )
    {
        ...
        (metadataURI, equippableGroupId, catalogAddress, partIds) = target_
            .getAssetAndEquippableData(tokenId, assetId);
        if (catalogAddress == address(0)) revert TicketingNotComposableAsset();
        ...
        if (len != 0) {
            ITicketingCatalog.Part[]
                memory catalogFixedParts = ITicketingCatalog(catalogAddress)
    ...
    function _getEquippableSlotsFromParent(
        ...
        for (uint256 i; i < totalMatches; ) {
            ...
            equippableData[i].partMetadata = ITicketingCatalog(
                parentAssetCatalog
        ...
        returns (EquippedSlotPart[] memory slotParts, uint256 totalEquipped)
    {
        ...
        if (len != 0) {
            ...
            ITicketingCatalog.Part[]
                memory catalogSlotParts = ITicketingCatalog(catalogAddress)
    ...
    {
        ITicketingCatalog.Part[] memory allParts = ITicketingCatalog(
            catalogAddress
        ).getParts(allPartIds);
        ...
        for (uint256 i; i < numParts; ) {
            if (allParts[i].itemType == ITicketingCatalog.ItemType.Fixed)
                numFixedParts += 1;
                // We could just take the numParts - numFixedParts, but it doesn't hurt to double check it's not an uninitialized part:
            else if (allParts[i].itemType == ITicketingCatalog.ItemType.Slot)
                numSlotParts += 1;
            unchecked {
                ++i;
            }
        ...
        for (uint256 i; i < numParts; ) {
            if (allParts[i].itemType == ITicketingCatalog.ItemType.Fixed) {
                ...
                allParts[i].itemType == ITicketingCatalog.ItemType.Slot
            ) {
