...
/*
 * Ticketing Equippables accessory contract, responsible for state storage and management of equippable items.
 */
...
import "../../interfaces/standards/composableNft/IERC6059.sol";
...
/**
 * @title TicketingExternalEquip
 *
 * @notice Smart contract of the Ticketing External Equippable module.
 * @dev This smart contract is expected to be paired with an instance of `TicketingNestableExternalEquip`.
 */
contract TicketingExternalEquip is
    Initializable,
    ReentrancyGuard,
    AbstractMultiAsset,
    ITicketingExternalEquip
{
    using TicketingLib for uint64[];
    ...
    function _onlyApprovedOrOwner(uint256 tokenId) internal view {
        if (
            !ITicketingNestableExternalEquip(_nestableAddress)
    ...
    function _onlyApprovedForAssetsOrOwner(uint256 tokenId) private view {
        if (!_isApprovedForAssetsOrOwner(_msgSender(), tokenId))
            revert TicketingNotApprovedForAssetsOrOwner();
    }
    ...
    function __TicketingExternalEquip_init(
        address nestableAddress,
        address forwarder_
    ) internal onlyInitializing {
        __TicketingExternalEquip_init_unchained(nestableAddress, forwarder_);
    }
     ...
     * @param nestableAddress Address of the Nestable module of external equip composite
     */
    function __TicketingExternalEquip_init_unchained(
        address nestableAddress,
        address forwarder_
    ) internal onlyInitializing {
        _setNestableAddress(nestableAddress);
        __AbstractMultiAsset_init(forwarder_);
    }
    ...
    function supportsInterface(
        ...
        return (interfaceId == type(ITicketingExternalEquip).interfaceId ||
    ...
    /**
     * @inheritdoc ITicketingExternalEquip
     */
    ...
    function approveForAssets(address to, uint256 tokenId) public virtual {
        address owner = ownerOf(tokenId);
        if (to == owner) revert TicketingApprovalForAssetsToCurrentOwner();
        ...
        ) revert TicketingApproveForAssetsCallerIsNotOwnerNorApprovedForAll();
    ...
    function _equip(IntakeEquip memory data) internal virtual {
        ...
        ) revert TicketingSlotAlreadyUsed();
        ...
        address childEquippable = ITicketingNestableExternalEquip(
        ...
        ) revert TicketingTokenCannotBeEquippedWithAssetIntoSlot();

        // Check from catalog perspective
        if (
            !ITicketingCatalog(catalogAddress).checkIsEquippable(
                slotPartId,
                childEquippable
            )
        ) revert TicketingEquippableEquipNotAllowedByCatalog();
    ...
    function _checkAssetAcceptsSlot(
        ...
        if (!found) revert TicketingTargetAssetCannotReceiveSlot();
    }
    ...
    function _setValidParentForEquippableGroup(
        ...
        if (equippableGroupId == uint64(0) || slotPartId == uint64(0))
            revert TicketingIdZeroForbidden();
    ...
    function _addAssetEntry(
        ...
        if (catalogAddress == address(0) && partIds.length != 0)
            revert TicketingCatalogRequiredForParts();
