...
import "../../interfaces/standards/composableNft/IERC5773.sol";
 ...
 */
abstract contract AbstractMultiAsset is Rbac, IERC5773 {
    ...
    /**
     * @notice Used to assign the  forwarder value
     * @param forwarder_ address of the MinimalForwarderUpgradeable
     */
    function __AbstractMultiAsset_init(
        address forwarder_
    ) internal onlyInitializing {
        __AbstractMultiAsset_init_unchained(forwarder_);
    }
     ...
     * @param forwarder_ address of the MinimalForwarderUpgradeable
     */
    function __AbstractMultiAsset_init_unchained(
        address forwarder_
    ) internal onlyInitializing {
        __Rbac_init(_msgSender(), forwarder_);
    }
    ...
    function getAssetMetadata(
        ...
        if (!_tokenAssets[tokenId][assetId])
            revert TicketingTokenDoesNotHaveAsset();
    ...
    function setApprovalForAllForAssets(
        ...
        if (_msgSender() == operator)
            revert TicketingApprovalForAssetsToCurrentOwner();
    ...
    function _validatePendingAssetAtIndex(
        ...
        if (index >= _pendingAssets[tokenId].length)
            revert TicketingIndexOutOfRange();
        if (assetId != _pendingAssets[tokenId][index])
            revert TicketingUnexpectedAssetId();
    }
    ...
    function _rejectAllAssets(
        ...
        if (len > maxRejections) revert TicketingUnexpectedNumberOfAssets();
    ...
    function _setPriority(
        ...
        if (length != _activeAssets[tokenId].length)
            revert TicketingBadPriorityListLength();
    ...
    function _addAssetEntry(
        ...
        if (id == uint64(0)) revert TicketingIdZeroForbidden();
        if (bytes(_assets[id]).length > 0) revert TicketingAssetAlreadyExists();
    ...
    function _addAssetToToken(
        ...
        if (_tokenAssets[tokenId][assetId])
            revert TicketingAssetAlreadyExists();

        if (bytes(_assets[assetId]).length == uint256(0))
            revert TicketingNoAssetMatchingId();

        if (_pendingAssets[tokenId].length >= 128)
            revert TicketingMaxPendingAssetsReached();
