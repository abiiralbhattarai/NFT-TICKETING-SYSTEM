    ...
    IERC6059
{
    ...
    mapping(uint256 => DirectOwner) private _TicketingOwners;
    ...
    function _onlyApprovedOrDirectOwner(uint256 tokenId) private view {
        if (!_isApprovedOrDirectOwner(_msgSender(), tokenId))
            revert TicketingNotApprovedOrDirectOwner();
    }
    ...
    function __TicketingNestable_init(
        string memory name_,
        string memory symbol_
    ) internal onlyInitializing {
        __TicketingNestable_init_unchained(name_, symbol_);
    }
     ...
     * @param symbol_ Symbol of the token collection
     */
    function __TicketingNestable_init_unchained(
        string memory name_,
        string memory symbol_
    ) internal onlyInitializing {
        __TicketingCore_init(name_, symbol_);
    }
    ...
    function _nestTransfer(
        ...
        if (to == address(this) && tokenId == destinationId)
            revert TicketingNestableTransferToSelf();

        // Destination contract checks:
        // It seems redundant, but otherwise it would revert with no error
        if (!to.isContract()) revert TicketingIsNotContract();
        if (
            !IERC165Upgradeable(to).supportsInterface(
                type(IERC6059).interfaceId
            )
        ) revert TicketingNestableTransferToNonTicketingNestableImplementer();
    ...
    function _checkForInheritanceLoop(
        ...
        for (uint256 i; i < _MAX_LEVELS_TO_CHECK_FOR_INHERITANCE_LOOP; ) {
            ...
            if (nextOwner == address(this) && nextOwnerTokenId == currentId) {
                revert TicketingNestableTransferToDescendant();
            }
        ...
        revert TicketingNestableTooDeep();
    }
    ...
    function _nestMint(
        ...
        ) revert TicketingMintToNonTicketingNestableImplementer();
    ...
    function _innerMint(
        ...
        if (tokenId == uint256(0)) revert TicketingIdZeroForbidden();
        ...
        _balances[to] += 1;
        _TicketingOwners[tokenId] = DirectOwner({
            ownerAddress: to,
    ...
    function directOwnerOf(
        ...
        DirectOwner memory owner = _TicketingOwners[tokenId];
    ...
    function _burn(
        ...
        for (uint256 i; i < length; ) {
            if (totalChildBurns >= maxChildrenBurns)
                revert TicketingMaxRecursiveBurnsReached(
        ...
        delete _TicketingOwners[tokenId];
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return _TicketingOwners[tokenId].ownerAddress != address(0);
    }
    ...
    function addChild(
        ...
        if (!childAddress.isContract()) revert TicketingIsNotContract();
        ...
        if (length < 128) {
            ...
            revert TicketingMaxPendingChildrenReached();
        }
     ...
     * @param childIndex Index of a child token in the given parent's pending children array
    ...
    function _acceptChild(
        ...
        if (_childIsInActive[childAddress][childId] != 0)
            revert TicketingChildAlreadyExists();
    ...
    function _rejectAllChildren(
        ...
        if (_pendingChildren[tokenId].length > maxRejections)
            revert TicketingUnexpectedNumberOfChildren();
    ...
    function _transferChild(
        ...
        if (to != address(0)) {
            if (destinationId == uint256(0)) {
                IERC721Upgradeable(childAddress).safeTransferFrom(
                    address(this),
                    to,
                    childId,
                    data
    ...
    function _checkExpectedChild(
        ...
        ) revert TicketingUnexpectedChildId();
    }
    ...
    function childOf(
        ...
        if (childrenOf(parentId).length <= index)
            revert TicketingChildIndexOutOfRange();
    ...
    function pendingChildOf(
        ...
        if (pendingChildrenOf(parentId).length <= index)
            revert TicketingPendingChildIndexOutOfRange();
